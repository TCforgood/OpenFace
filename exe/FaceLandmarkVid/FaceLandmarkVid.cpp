...
int main(int argc, char **argv)
{
	...
	while (true) // this is not a for loop as we might also be reading from a webcam
	{
		...
		while (!rgb_image.empty()) // this is not a for loop as we might also be reading from a webcam
		{
			...
			cv::Point3f leftEyeball(0, 0, 1);
			cv::Point3f rightEyeball(0, 0, 1);

			// If tracking succeeded and we have an eye model, estimate gaze
			if (detection_success && face_model.eye_model)
			{
				GazeAnalysis::EstimateGaze(face_model, gazeDirection0, leftEyeball, sequence_reader.fx, sequence_reader.fy, sequence_reader.cx, sequence_reader.cy, true);
				GazeAnalysis::EstimateGaze(face_model, gazeDirection1, rightEyeball, sequence_reader.fx, sequence_reader.fy, sequence_reader.cx, sequence_reader.cy, false);
			}

			// Work out the pose of the head from the tracked model
			cv::Vec6d pose_estimate = LandmarkDetector::GetPose(face_model, sequence_reader.fx, sequence_reader.fy, sequence_reader.cx, sequence_reader.cy);

			// Calculate corresponding screen location
			cv::Point3f midEye = (leftEyeball + rightEyeball) / 2.0f;
			cv::Vec2f angle = GazeAnalysis::GetGazeAngle(gazeDirection0, gazeDirection1);
			std::cout << "mid eye: " << midEye << std::endl;
			std::cout << "gaze angle: " << gazeDirection0 << std::endl;
			std::cout << "screen x: " << midEye.z * tan(gazeDirection0.x) << std::endl;
			std::cout << "screen y: " << midEye.z * tan(gazeDirection0.y) << std::endl;
			...
			// std::cout << "pose: " << pose_estimate << std::endl;
			// std::cout << "gaze dir: " << gazeDirection0 << std::endl;
